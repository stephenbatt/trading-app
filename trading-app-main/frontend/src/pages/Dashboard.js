...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        if (response.data.symbol) setSymbol(response.data.symbol);
        if (response.data.interval) setIntervalValue(response.data.interval);
      } catch (error) {
  ...
  const fetchStockData = useCallback(async () => {
    try {
      ...
      const [indicatorsRes, signalRes] = await Promise.all([
        stocks.getIndicators(symbol, params),
        stocks.getSignal(symbol, params),
      ]);
      setStockData(indicatorsRes.data);
  ...
  useEffect(() => {
    ...
    fetchTrades();
    const tradesIntervalId = setInterval(fetchTrades, 30000);
    return () => clearInterval(tradesIntervalId);
  }, [symbol]);
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      toast.success('Settings saved — the auto-trader will use these numbers');
    } catch (error) {
  ...
  const handleToggleStrategy = async (enabled) => {
    try {
      ...
      toast.success(
        enabled
          ? `Auto-trading is ON for ${symbol} (${interval}) — runs in the background even if you close this page`
  ...
  const signalStyles = {
    BUY: {
      card: 'bg-green-500/10 border-green-500/50',
  ...
  const activeSignal = currentSignal?.signal || 'HOLD';
  const signalStyle = signalStyles[activeSignal] || signalStyles.HOLD;
  ...
  return (
    <Layout>
      <div className="p-4 space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Label className="text-sm text-zinc-400 font-medium">Timeframe:</Label>
              ...
              <div className="flex items-center gap-3 px-4 py-2 rounded-lg bg-zinc-800 border border-zinc-700">
                ...
                <Label htmlFor="strategy-toggle" className="text-sm text-zinc-300">Strategy</Label>
                <Switch
                  id="strategy-toggle"
        ...
        {/* Current Signal - what the auto-trader is seeing right now */}
        <Card className={`border-2 ${signalStyle.card}`}>
          <CardContent className="p-4 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-4">
              <div className={`px-4 py-2 rounded-lg font-heading text-xl font-bold ${signalStyle.pill}`} data-testid="current-signal-pill">
                {activeSignal}
              </div>
              <div>
                <p className="text-sm text-zinc-400">
                  {currentSignal?.reason || 'Waiting for a clean fast-EMA bounce + CCI/MACD confirmation...'}
                </p>
                <p className="text-xs text-zinc-500 font-mono mt-1">
                  Checked candle: {currentSignal?.candle_time || '-'}
                </p>
              </div>
            </div>
            {userSettings.strategy_enabled && (
              <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/30">
                Auto-trading ON — checking every minute in the background
              </Badge>
            )}
          </CardContent>
        </Card>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <Badge variant="outline" className="border-zinc-700 text-zinc-400">{interval}</Badge>
                </div>
                {loading ? (
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className="col-span-12 lg:col-span-9 space-y-4">
            {/* Candlestick Chart - single chart, all 3 EMAs, fixed scaling */}
            <Card className="bg-zinc-900 border-zinc-800">
            ...
            {/* MACD Chart - now with colored histogram bars */}
            <Card className="bg-zinc-900 border-zinc-800">
              <CardContent className="p-2">
                {loading ? (
          ...
          <div className="col-span-12 lg:col-span-3 space-y-4">
            <Card className="bg-zinc-900 border-zinc-800">
              ...
              <CardContent className="space-y-6">
                ...
                >
                  {(refreshing || savingSettings) ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}
                  Apply Settings
                </Button>
              </CardContent>
            </Card>
                ...
                {openTrades.length > 0 && (
                  <div className="space-y-2 pt-4 border-t border-zinc-800">
                    ...
                    {openTrades.map((trade) => (
                      <div key={trade.id} className="flex items-center justify-between p-2 rounded-lg bg-zinc-800">
                        <div>
                          <div className="flex items-center gap-2">
                            <Badge
